...
namespace Unity.LiveCapture.VideoStreaming.Server
{
    ...
    class Authentication
    {
        ...
        public Authentication(String username, String password, String realm, Type authentication_type)
        {
            ...
            this.nonce = new System.Random().Next(100000000, 999999999).ToString(); // random 9 digit number
        }
        ...
        public bool IsValid(Messages.RtspMessage received_message)
        {
            ...
            if (authentication_type == Type.Basic && authorization.StartsWith("Basic "))
            {
                ...
                {
                    _logger.Log("Basic Authorization passed");
                    return true;
                }
                else
                {
                    _logger.Log("Basic Authorization failed");
                    return false;
                }
            }
            ...
            if (authentication_type == Type.Digest && authorization.StartsWith("Digest "))
            {
                ...
                {
                    _logger.Log("Digest Authorization passed");
                    return true;
                }
                else
                {
                    _logger.Log("Digest Authorization failed");
                    return false;
                }
            }
            return false;
