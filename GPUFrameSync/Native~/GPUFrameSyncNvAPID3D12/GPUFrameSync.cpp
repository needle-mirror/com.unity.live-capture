...
{
#ifdef DEBUG_LOG
	void GPUFrameSync::WriteFileDebug(const char* const message,
		const bool append)
	{
		std::ofstream myfile;
	...
	bool GPUFrameSync::Initialize(ID3D12Device* const pDevice, IDXGISwapChain* const pSwapChain)
	{
	...
	void GPUFrameSync::Dispose(ID3D12Device* const pDevice, IDXGISwapChain* const pSwapChain)
	{
		NvAPI_Status status;
		if (m_GroupId > 0)
		{
	...
	NvU32 GPUFrameSync::QueryFrameCount(ID3D12Device* const pDevice)
	{
		NvU32 count = 0;
	...
	void GPUFrameSync::ResetFrameCount(ID3D12Device* const pDevice)
	{
		if (m_GSyncMaster)
		{
			auto status = NVAPI_OK;
	...
	bool GPUFrameSync::Render(ID3D12Device* const pDevice,
	...
	void GPUFrameSync::EnableSystem(ID3D12Device* const pDevice,
		IDXGISwapChain* const pSwapChain,
		const bool value)
	{
		m_IsActive = value;
	...
	void GPUFrameSync::EnableSwapGroup(ID3D12Device* const pDevice,
		                               IDXGISwapChain* const pSwapChain,
		                               const bool value)
	{
#ifdef DEBUG_LOG
	...
	void GPUFrameSync::EnableSwapBarrier(ID3D12Device* const pDevice, const bool value)
	{
		if (m_GroupId == 1)
		{
#ifdef DEBUG_LOG
